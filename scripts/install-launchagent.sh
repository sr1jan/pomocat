#!/usr/bin/env bash
# install-launchagent.sh — Install pomocat as a per-user LaunchAgent.
# pomocat starts at every login and restarts automatically on crash.
#
...
BINARY="$REPO_ROOT/.build/release/pomocat"
...
echo "  Logs:    $REPO_ROOT/.pomocat.log"
echo "  Binary:  $BINARY"
echo
echo "pomocat is now running and will restart at every login."
